package sound
{
	...
	public class SoundPlayer
	{
		...
		{
			for (var i:int = 0; i < AmbianceChannels.length; i++)
			{
				ambianceChannel = AmbianceChannels[i];
		 ...
		 */
		public static function playAmbiance():void
		{
			for (var i:int = 0; i < Ambiances.length; i++)
			{
				trace("Playing ambiance : " + Ambiances[i]);
				ambiance = new Ambiances[i]() as Sound;
				AmbianceChannels[i] = ambiance.play(0, int.MAX_VALUE, new SoundTransform(VolumeAmbiance, 0));
			}
			//playAmbiance();
		}
		...
		public static function increaseLevel():void
		{
			stopAmbiance();
			AmbienceIndex++;
			trace("Playing ambiance : " + AmbienceIndex);
			Ambiances = Ambiancess[AmbienceIndex];
			playAmbiance();
		}
